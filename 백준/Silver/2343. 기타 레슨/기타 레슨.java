...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        long left = 1L; // 초기 블루레이 하한값
        long right = 0L; // 초기 블루레이 상한값
        ...
        for(int idx = 0; idx < N; idx++) {
            ...
            right += records[idx];
            left = Math.max(left, records[idx]);
        }

        long ans = Long.MAX_VALUE;
        // 이진탐색 시작
        while (left <= right) {
            long mid = (left + right) / 2; // 확인하고자 하는 블루레이 크기
    ...
    private static int getDisckCount(long diskSize) {
        int diskCnt = 0;
        long currDiskStatus = 0; // 현재 블루레이에 대해 사용한 용량(size) 카운팅
